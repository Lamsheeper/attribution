...
def find_olmo_files(input_dir):
    ...
    """
    # Look for both stage 1 and stage 2 files
    patterns = [
        "OLMo-2-1124-7B_stage1-step*-tokens*B.json",  # Stage 1 pattern
        "OLMo-2-1124-7B_stage2-ingredient*-step*-tokens*B.json"  # Stage 2 pattern
    ]
...
def process_files(input_files, output_dir, filter_script):
    ...
    for input_file in input_files:
        ...
        # Determine if this is a stage 1 or stage 2 file
        if "stage1" in base_name:
            # Stage 1 file - extract tokens part
            match = re.search(r'(tokens\d+B)', base_name)
            if match:
                tokens_part = match.group(1)
                output_filename = f"stage1-{tokens_part}.json"
            else:
                output_filename = f"filtered_{base_name}"
        elif "stage2" in base_name:
            # Stage 2 file - extract ingredient number and tokens part
            ingredient_match = re.search(r'ingredient(\d+)', base_name)
            tokens_match = re.search(r'(tokens\d+B)', base_name)
            
            if ingredient_match and tokens_match:
                ingredient_num = ingredient_match.group(1)
                tokens_part = tokens_match.group(1)
                output_filename = f"stage2-ingredient{ingredient_num}-{tokens_part}.json"
            else:
                output_filename = f"filtered_{base_name}"
        else:
            # Unknown format
