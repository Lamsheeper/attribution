...
def main():
    ...
    # Load results
    print(f"Loading standard results from {standard_file}")
    standard_results = load_results(standard_file)
    print(f"Loading quantized results from {quantized_file}")
    quantized_results = load_results(quantized_file)
    
    # Check if data loaded successfully
    if not standard_results or not quantized_results:
        print("Error: Failed to load one or both result files.")
        return
    
    print(f"Loaded {len(standard_results)} standard results and {len(quantized_results)} quantized results.")
    
    # Generate comparison summary
    summary = generate_comparison_summary(standard_results, quantized_results, output_file)
    
    # Print key findings
    print("\nKey Findings:")
    print(f"Total problems with at least one correct solution: {summary['total_problems_with_any_correct']}")
    print(f"Standard model correct: {summary['standard_correct_count']} problems")
    print(f"Quantized model correct: {summary['quantized_correct_count']} problems")
    print(f"Both models correct: {summary['both_correct']} problems")
    print(f"Only standard model correct: {summary['only_standard_correct']} problems")
    print(f"Only quantized model correct: {summary['only_quantized_correct']} problems")
    print(f"Avg standard correct rate: {summary['average_standard_correct_rate']:.4f}")
    print(f"Avg quantized correct rate: {summary['average_quantized_correct_rate']:.4f}")
